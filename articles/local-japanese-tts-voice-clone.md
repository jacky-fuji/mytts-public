...
![ローカル日本語TTS / Voice Clone検証 プロジェクト全体図](/images/local-japanese-tts-voice-clone/tts_voice_clone_project_map_16x9_1920.png)

## このレポートの位置づけ

本稿は、個人が自分自身の声だけを対象に行ったローカル環境での検証記録である。査読済み論文、製品評価レポート、第三者機関によるベンチマーク、専門家の鑑定ではない。
...
:::message
音声サンプルはGitHub Pages側の試聴ページに分離する予定です。Zenn本文は検証手順、結果、音響分析を中心にしています。
:::
...
|---|---|---|
| `similarity_h4n_005` | natural_numbers | はい、では次に、数字と固有名詞を含む文章をもう一度確認しましょう。 |
...
| Run | Model | 生成数 | 生成音声の合計 | Wall time | RTF目安 | コメント |
|---|---|---:|---:|---:|---:|---|
| EV launch | Irodori-TTS 600M VoiceDesign | 40 | 401.25s | 561.4s | 約1.4x | 長めのプレゼン生成に向く |
| H4n presentation | Irodori-TTS | 12 | 約115.1s | 164.3s | 約1.4x | 500Mと600Mを両方生成 |
| Same-text | VoxCPM2 | 8 | 約45.1s | 73.5s | 約1.6x | cloneとultimateを生成 |
| Same-text | CosyVoice2 | 4 | 約23.9s | 44.4s | 約1.9x | prompt/target長の警告あり |
| H4n presentation | Qwen3-TTS 1.7B | 6 | 約48.2s | 112.3s | 約2.3x | 短文なら実用的 |
| H4n presentation | Fish Speech S2 Pro | 6 | 約48.8s | 5253.3s | 約107.7x | 高品質だが非常に遅い |

生成音声の合計長がRunごとに異なるため、下の横棒グラフはWall timeそのものではなくRTF目安で並べた。短いほど速い。Fish Speechが外れ値として大きいため、横軸は対数目盛りにしている。
...
Irodori-TTS 600M VoiceDesignは、速度と品質のバランスが良い。40チャンク、約6分41秒の音声を561.4秒で生成できた。
...
弱点は生成速度である。約49秒の音声生成に約87.6分かかっており、日常的に多数のパラメータを振る用途には重い。
...
VoiceDesignのcaptionで、落ち着いた声、技術説明向けの明瞭な声、力強い発表口調などを制御できる。声質の完全一致ではFish Speechに及ばないが、生成速度、安定性、日本語向けの扱いやすさが強い。
...
軽く扱いやすい。Timbre 0.9745とFormants 0.8787は高く、音色系の特徴は悪くない。
...
詳細な数値、TSV、図表、分析スクリプトは以下に置いた。

- `docs/comparisons/acoustic_similarity_deep_report_ja.md`
- `docs/comparisons/acoustic_similarity_deep_metrics.tsv`
- `docs/comparisons/acoustic_similarity_segment_metrics.tsv`
- `docs/comparisons/acoustic_similarity_worst_regions.tsv`
- `images/acoustic_similarity_deep/`
- `scripts/analyze_acoustic_similarity_deep.py`
- `docs/comparisons/phonetic_micro_report_ja.md`
- `docs/comparisons/phonetic_micro_metrics.tsv`
- `docs/comparisons/phonetic_micro_summary.tsv`
- `docs/comparisons/phonetic_micro_worst_units.tsv`
- `images/acoustic_phonetic_micro/`
- `scripts/analyze_phonetic_micro_similarity.py`
